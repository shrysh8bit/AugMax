...
train_aug_indices = np.load('./mnist_train_aug_indices.npy')
...
class AugMaxDataset(torch.utils.data.Dataset):
	...
	def __getitem__(self, i):
		...
		for _ in range(self.img_tuple_num):
			...
			#print(len(img_tuples))
			#print(len(label_tuples))

		#print("Not entered")
		return img_tuples, y
		#return img_tuples, label_tuples    
...
class TrajSel(object):
	...
	def mnist_dataloaders(self,data_dir, num_classes=10, AugMax=None, **AugMax_args):
    
		assert num_classes in [10, 100]
		MNIST = datasets.MNIST if num_classes == 10 else datasets.CIFAR100
	...
	def aug(self,image, preprocess, mixture_depth, aug_severity):
		...
		#print(transform(image_aug))
		#exit(1)
		#print(image_aug.type)
		#print(image_aug)
		depth = mixture_depth if mixture_depth > 0 else np.random.randint(1, 4)
		for _ in range(depth):
		    op = np.random.choice(aug_list)
		    ...
		    #transform = transforms.ToPILImage()
		    #print(transform(image_aug.squeeze(0)).shape)
		    image_aug = op(image_aug, aug_severity)
		    #print(image_aug)
	...
	def fit(self):
		...
		#train_data, val_data = self.mnist_dataloaders(data_dir='./data/', num_classes=10,AugMax=AugMaxDataset, mixture_width=3, mixture_depth=-1, aug_severity=3)
		#print('Length of augmented data')
		#print(len(train_data))
		#exit(1)    
		print(len(ftrain))
		print(len(ytrain))
        ...
        #self.trainloader = torch.utils.data.DataLoader(train_data,batch_sampler=sampler)
