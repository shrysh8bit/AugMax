...
class TrajSel(object):
	...
	def fit(self):
		...
		for batchid, (ip, targ) in enumerate(trainl):
			...
			for _ in range(1):
				ftrain.append(np.expand_dims(self.aug(ip, test_transform, 3, 3).cpu().numpy(),axis=0)) #changed
		...
		for epoch in range(self.start_epoch,self.start_epoch+self.epochs):
			...
			self.helpobj.checksel_cb(lossval,czs,self.model,epoch,netv)
		dvalueobj = DataValueCheckSel(self.trainset,self.test_loader,self.model,self.helpobj,self.confdata)
